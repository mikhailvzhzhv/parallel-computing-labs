...
void check_vector_stop(char* vector, const int rows, const int cols, int* flag);
...
int main(int argc, char* argv[]) {
    ...
    char* vector_stop_buf = new char[len_vector * size];
    ...
    for (gen = 0; gen < max_gen && !stop; ++gen) {
        ...
        MPI_Iallgather(vector_stop, len_vector, MPI_CHAR, vector_stop_buf, len_vector, MPI_CHAR, MPI_COMM_WORLD, &req_vector);
        ...
        check_vector_stop(vector_stop_buf, size, len_vector, &flag);

        MPI_Allreduce(&flag, &stop, 1, MPI_INT, MPI_LAND, MPI_COMM_WORLD);
...
void check_neighbours(char* arr_origin, char* arr_copy, const int cols, const int rows) {
    ...
    for (int row = 1; row < rows - 1; ++row) {
        for (int col = 0; col < cols; ++col) {
            ...
            if (is_alive(arr_origin[(idx + 1) % cols + cols * row])) {
                arr_copy[idx] += 1;
            }
            if (is_alive(arr_origin[(idx - 1) % cols + cols * row])) {
                arr_copy[idx] += 1;
            }
            if (is_alive(arr_origin[idx_top])) {
                arr_copy[idx] += 1;
            }
            if (is_alive(arr_origin[(idx_top + 1) % cols + cols * (row - 1)])) {
                arr_copy[idx] += 1;
            }
            if (is_alive(arr_origin[(idx_top + cols - 1) % cols + cols * (row - 1)])) {
                arr_copy[idx] += 1;
            }
            if (is_alive(arr_origin[idx_bot])) {
                arr_copy[idx] += 1;
            }
            if (is_alive(arr_origin[(idx_bot + 1) % cols + cols * (row + 1)])) {
                arr_copy[idx] += 1;
            }
            if (is_alive(arr_origin[(idx_bot - 1) % cols + cols * (row + 1)])) {
                arr_copy[idx] += 1;
...
void check_neighbours_line(char* arr_origin, char* arr_copy, char* arr_of_cells_pos, const int cols, const int offset, int mode) {
    ...
    for(int col = 0; col < cols; ++col) {
        idx = offset + col;
        if (is_alive(arr_origin[(idx + 1) % cols + offset])) {
            arr_copy[idx] += 1;
        }
        if (is_alive(arr_origin[(idx + cols - 1) % cols + offset])) {
            arr_copy[idx] += 1;
        }
        if (is_alive(arr_of_cells_pos[col])) {
            arr_copy[idx] += 1;
        }
        if (is_alive(arr_of_cells_pos[(col + 1) % cols])) {
            arr_copy[idx] += 1;
        }
        if (is_alive(arr_of_cells_pos[(col + cols - 1) % cols])) {
            arr_copy[idx] += 1;
        }
        if (is_alive(arr_origin[idx + cols * mode])) {
            arr_copy[idx] += 1;
        }
        if (is_alive(arr_origin[(col + cols + 1) % cols + cols * mode + offset])) {
            arr_copy[idx] += 1;
        }
        if (is_alive(arr_origin[(col + cols - 1) % cols + cols * mode + offset])) {
            arr_copy[idx] += 1;
...
void update_world(char* arr_origin, char* arr_copy, const int cols, const int rows) {
    for (int i = 0; i < rows * cols; ++i) {
        if (is_alive(arr_origin[i]) && (arr_copy[i] == 2 || arr_copy[i] == 3)) {
            set_life(arr_copy + i);
        } else if (!is_alive(arr_origin[i]) && arr_copy[i] == 3) {
            set_life(arr_copy + i);
        }
    }
}
